...
void *grabRsrcs(void *threadp)
{
   ...
   while (!acquiredBoth)
   {
      if(threadIdx == THREAD_1)
      {
         ...
         {
            printf("THREAD 1 failed to get B, backing off\n");
            pthread_mutex_unlock(&rsrcA);
            usleep((rand() % 10 + 1)*1000); // Sleep for 1 to 10 ms
         }
      }
      else
      {
         ...
         {
            printf("THREAD 2 failed to get A, backing off\n");
